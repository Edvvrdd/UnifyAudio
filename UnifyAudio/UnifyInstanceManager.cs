...
namespace UnifyAudio
{
    public class UnifyInstanceManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            {
                Debug.LogWarning($"[UnifyAudio] Failed to create FMOD instance for '{name}'. Ensure the event reference is valid.");
                return;
            }
        ...
        public void Pause()
        {
            if (!_instance.isValid())
            {
                Debug.LogWarning($"[UnifyAudio] Pause called on '{name}' but instance is not valid.");
                return;
            }
        ...
        public void Stop()
        {
            if (!_instance.isValid())
            {
                Debug.LogWarning($"[UnifyAudio] Stop called on '{name}' but instance is not valid.");
                return;
            }
            _instance.stop(FMOD.Studio.STOP_MODE.ALLOWFADEOUT);
        ...
        public FMOD.RESULT ApplyParameter(string parameterName, float value)
        {
            if (!_instance.isValid())
            {
                Debug.LogWarning($"[UnifyAudio] ApplyParameter called on '{name}' but instance is not valid.");
